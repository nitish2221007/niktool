...
const indexMap = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'assets', 'data', 'tools-index.json'), 'utf8'));

const workerTemplate = `function escapeHtml(str) {
  if (!str) return '';
  return str
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
const TOOLS_INDEX = ${JSON.stringify(indexMap)};
...
const functionsWorker = workerTemplate + `
...
const rootWorker = workerTemplate + `
export default {
...
fs.writeFileSync(path.join(__dirname, '..', 'functions', 'tools', '[[path]].js'), functionsWorker);
fs.writeFileSync(path.join(__dirname, '..', '_worker.js'), rootWorker);
console.log('Successfully compiled functions/tools/[[path]].js and _worker.js!');
